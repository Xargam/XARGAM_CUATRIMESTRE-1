 ...
 * \param dataLength : [CUIDADO] Longitud en caracteres de los datos a leer. Se recomienda utilizar valores amplios a fin de evitar crashes.
 ...
 *
 */
void xlkSortPrintf(int acotado,int dataLength,char* dataType,int lines,...)
{
    if(dataType != NULL && lines > -1 && lines < 3 && acotado <= 74 && acotado >= -74)
    {
        ...
        char dataToPrint[6][dataLength]; //MATRIZ QUE VA GUARDANDO LOS ARGUMENTOS LEIDOS PARA LUEGO IMPRIMIRLOS.
        char dataToPrintAux[dataLength]; //AUXILIAR UTILIZADO PARA COPIAS.
