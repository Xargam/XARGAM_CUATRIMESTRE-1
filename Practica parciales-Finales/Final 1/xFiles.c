 ...
 * \param fileType : Puede ser la cadena "bin" o "txt" acorde al tipo de archivo con el que se trabaja.
...
int xfilFileSync(char* filePath,  char* fileType)
{
    ...
    if(filePath != NULL && fileType != NULL)
    {
        ...
        if( !stricmp(fileType, "bin") )
        {
            ...
            else if( (file = fopen(filePath, "wb" )) != NULL )
            {
                verify = 0;
            }
        }
        else if( !stricmp(fileType, "txt") )
        {
            if( (file = fopen(filePath, "r")) != NULL )
            {
                verify = 1;
            }
            else if( (file = fopen(filePath, "w" )) != NULL )
            {
                verify = 0;
            }
        }
        if( verify > -1 && fclose(file) )
        {
            verify = -1;
        }
