...
int get_validateIsNum(char* number)
{
    int verify = 1;
    int i;
...
int get_validateIsRealNum(char* number)
{
...
int get_validateIsAlphabeticStr(char* word)
{
    int verify = 1;
    int i;
    for(i = 0 ; *(word + i) != '\0' ; i++ )
...
int get_validateIsAlphanumericStr(char* word)
{
    int verify = 1;
    int i;
    for(i = 0 ; word[i] != '\0' ; i++ )
...
int get_validateIsGraphicStr(char* word)
{
    int verify = 1;
    int i;
    for(i = 0 ; *(word + i) != '\0' ; i++ )
...
void get_zeroFixer(char* number)
{
    int indicePunto = -1;
    int flag = 0;
...
int get_strNumCmp(char* number1, char* number2)
{
    get_zeroFixer(number1);
    get_zeroFixer(number2);
...
//XG1-5
/** \brief Pide un string al usuario con cantidad de caracteres especificada y lo guarda si es valido.
 *
 * \param word : cadaena de caracteres donde se guardara la cadena pedida.
 * \param minChars : Cantidad minima de caracteres que puede contener la cadena.
...
int getRangedStr(char* word, int minChars,int maxChars,char* message,char* eMessage, int mode)
{
    ...
    if( eMessage != NULL && message != NULL && word != NULL)
    {
        ...
        int chars = strlen(auxWord);

        if( chars > maxChars || chars < minChars)
        {
            printf("** %s\n\n",eMessage);
        }
        else
...
//XG1-6
/** \brief Pide un string de caracteres alfabeticos con una longitud especificada y lo guarda solo si es valido.
 *
...
int getRangedAlphaStr(char* word,int minChars, int maxChars, char *message,char *eMessage,int mode)
{
    ...
    if(message != NULL && eMessage != NULL)
    {
        ...
        if(chars > maxChars || chars < minChars || !get_validateIsAlphabeticStr(auxWord))
        {
            printf("** %s\n\n",eMessage);
            verify = 0;
        }
...
//XG1-6.1
/** \brief Pide un string de caracteres alfaNumericos con una longitud especificada y lo guarda solo si es valido.
 *
...
int getRangedAlphaNumStr(char* word,int minChars, int maxChars, char* message, char* eMessage, int mode)    //XG1-6.1
{
    ...
    if(chars > maxChars || chars < minChars || !get_validateIsAlphanumericStr(auxWord))
    {
        printf("** %s\n\n",eMessage);
        verify = 0;
    }
...
//XG1-6.2
/** \brief Pide un string de caracteres imprimibles con longitud especificada y lo guarda solo si es valido.
 *
 * \param word: Direccion de memoria de un puntero donde guardar el string.
 * \param minChars: Cantidad minima de caracteres que puede contener la cadena.
 * \param maxChars: Cantidad maxima de caracteres que puede tener la cadena.
...
int getRangedGraphicStr(char* word,int minChars, int maxChars, char* message, char* eMessage, int mode)
{
    ...
    if(chars > maxChars || chars < minChars || !get_validateIsGraphicStr(auxWord))
    {
        printf("** %s\n\n",eMessage);
        verify = 0;
    }
...
int getValidInt(int* number, char message[], char errorMessage[])
{
    ...
    if(!get_validateIsNum(num)|| get_strNumCmp(num,"32767") > 0 || get_strNumCmp(num,"-32768") < 0)
    {
        printf("** %s\n\n",errorMessage);
        verificacion = 0;
    }
...
int getValidLong(long* number, char* message, char* errorMessage)
{
    ...
    if( !get_validateIsNum(num)|| get_strNumCmp(num,"2147483647") > 0 || get_strNumCmp(num,"-2147483648") < 0)
    {
        printf("** %s\n\n",errorMessage);
        verificacion = 0;
    }
...
int getValidFloat(float* number, char* message, char* errorMessage)
{
    ...
    if( !get_validateIsRealNum(num)|| get_strNumCmp(num,"10000000") > 0 || get_strNumCmp(num,"-10000000") < 0)
    {
        printf("** %s\n\n",errorMessage);
        verificacion = 0;
    }
...
int getValidLongLong(long long* number, char* message, char* errorMessage)
{
    ...
    if( !get_validateIsNum(num)|| get_strNumCmp(num,"9223372036854775807") > 0 || get_strNumCmp(num,"-9223372036854775808") < 0)
    {
        printf("** %s\n\n",errorMessage);
        verificacion = 0;
    }
